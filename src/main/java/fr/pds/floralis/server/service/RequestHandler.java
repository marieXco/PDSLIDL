...
/**
 * RequestHandler This class is made to read the client request and to answer it
 * 
 * @author alveslaura
 *
 */
...
public class RequestHandler implements Runnable {
	 ...
	 * @throws SQLException
	 */
	public RequestHandler(Socket pSock, JDBCConnectionPool jdbc,
	...
	/**
	 * run() Class that does the request traitment, called on a thread
	 */
	public void run() {
		...
		while (!sock.isClosed()) {
			try {
				...
				JSONObject parameters = requestJson
						.getJSONObject("requested-parameters");

				InetSocketAddress remote = (InetSocketAddress) sock
						.getRemoteSocketAddress();

				/**
				 * Debug infos remote contains the port and the ip address that
				 * made the request
				 */
				String debug = "";
				debug = "Address asking : "
				...
				case "SENSOR":
					...
					case "FINDBYID":
						Sensor sensorFound = sensorDao.find(parameters
								.getInt("id"));
					...
					case "CREATE":
						Sensor sensorToCreate = objectMapper.readValue(
								parameters.toString(), Sensor.class);
						Boolean sensorCreation = sensorDao
								.create(sensorToCreate);

						toSend = sensorCreation.toString();
						break;

					case "DELETE":
						Boolean sensorErasing = sensorDao.delete(parameters
								.getInt("id"));
					...
					case "UPDATE":
						Sensor sensorToUpdate = objectMapper.readValue(
								parameters.get("sensorToUpdate").toString(),
								Sensor.class);

						Boolean sensorUpdating = sensorDao.update(
								parameters.getInt("id"), sensorToUpdate);
					...
					case "FINDBYCONFIG":
						List<Sensor> sensorListConfig = sensorDao
								.findByConfig(parameters
										.getBoolean("configure"));

						JSONArray sensorFoundByConfig = new JSONArray(
								sensorListConfig);
					...
					case "FINDBYTYPE":
						List<Sensor> sensorListType = sensorDao
								.findByType(parameters.getString("type"));
						JSONArray sensorFoundByType = new JSONArray(
								sensorListType);
						toSend = sensorFoundByType.toString();
						break;

					case "FINDBYSTATE":
						List<Sensor> sensorListState = sensorDao
								.findByState(parameters.getBoolean("state"));
						JSONArray sensorFoundByState = new JSONArray(
								sensorListState);
						toSend = sensorFoundByState.toString();
						break;
				...
				case "LOCATION":
					...
					case "FINDBYID":
						Location locationFound = locationDao.find(parameters
								.getInt("id"));
					...
					case "CREATE":
						Location locationToCreate = objectMapper.readValue(
								parameters.toString(), Location.class);

						Boolean locationCreation = locationDao
					...
					case "DELETE":
						Boolean locationDeleting = locationDao
								.delete(parameters.getInt("id"));

						toSend = locationDeleting.toString();
						break;

					case "UPDATE":
						Location locationToUpdate = objectMapper.readValue(
								parameters.get("locationToUpdate").toString(),
								Location.class);

						Boolean locationUpdating = locationDao.update(
								parameters.getInt("id"), locationToUpdate);
				...
				case "ROOM":
					...
					case "CREATE":
						Room roomToCreate = objectMapper.readValue(
								parameters.toString(), Room.class);
					...
					case "DELETE":
						Boolean roomErasing = roomDao.delete(parameters
								.getInt("id"));
					...
					case "UPDATE":
						Room roomToUpdate = objectMapper.readValue(parameters
								.get("roomToUpdate").toString(), Room.class);

						Boolean roomUpdating = roomDao.update(
								parameters.getInt("id"), roomToUpdate);
				...
				case "BUILDING":
					...
					case "FINDBYID":
						Building buildingFound = buildingDao.find(parameters
								.getInt("id"));
					...
					case "CREATE":
						Building buildingToCreate = objectMapper.readValue(
								parameters.toString(), Building.class);

						Boolean buildingCreation = buildingDao
								.create(buildingToCreate);

						toSend = buildingCreation.toString();
						break;

					case "DELETE":
						Boolean buildingErasing = buildingDao.delete(parameters
								.getInt("id"));
					...
					case "UPDATE":
						Building buildingToUpdate = objectMapper.readValue(
								parameters.get("buildingToUpdate").toString(),
								Building.class);

						Boolean buildingUpdating = buildingDao.update(
								parameters.getInt("id"), buildingToUpdate);
				...
				case "PATIENTS":
					...
					case "FINDBYID":
						Patient patientFound = patientDao.find(parameters
								.getInt("id"));
						toSend = patientFound.toJSON().toString();
						break;

					case "CREATE":
						Patient patientToCreate = objectMapper.readValue(
								parameters.toString(), Patient.class);
						Boolean patientCreate = patientDao
								.create(patientToCreate);
						toSend = patientCreate.toString();
						break;

					case "DELETE":
						Boolean patientDelete = patientDao.delete(parameters
								.getInt("id"));
						toSend = patientDelete.toString();
						break;

					case "UPDATE":
						Patient patientToUpdate = objectMapper.readValue(
								parameters.get("patientToUpdate").toString(),
								Patient.class);
						Boolean patientUpdate = patientDao.update(
								parameters.getInt("id"), patientToUpdate);
				...
				case "HISTORY_ALERTS":
					...
					case "FINDBYID":
						Alert alertFound = alertDao.find(parameters
								.getInt("id"));
						toSend = alertFound.toJSON().toString();
						break;

					case "CREATE":
						Alert alertToCreate = objectMapper.readValue(
								parameters.toString(), Alert.class);
						Boolean alertCreate = alertDao.create(alertToCreate);
						toSend = alertCreate.toString();
						break;

					case "DELETE":
						Boolean alertDelete = alertDao.delete(parameters
								.getInt("id"));
						toSend = alertDelete.toString();
						break;

					case "UPDATE":
						Alert alertToUpdate = objectMapper.readValue(parameters
								.get("alertToUpdate").toString(), Alert.class);
						Boolean alertUpdate = alertDao.update(
								parameters.getInt("id"), alertToUpdate);
				...
				case "FLOOR":
					...
					case "FINDBYID":
						Floor floorFound = floorDao.find(parameters
								.getInt("id"));
					...
					case "CREATE":
						Floor floorToCreate = objectMapper.readValue(
								parameters.toString(), Floor.class);
					...
					case "DELETE":
						Boolean floorDelete = floorDao.delete(parameters
								.getInt("id"));
					...
					case "UPDATE":
						Floor floorToUpdate = objectMapper.readValue(parameters
								.get("floorToUpdate").toString(), Floor.class);

						Boolean floorUpdate = floorDao.update(
								parameters.getInt("id"), floorToUpdate);
				...
				case "TYPESENSOR":
					...
					switch (command.toUpperCase()) {
					case "FINDALL":
						List<TypeSensor> typeSensorList = typeSensorDao
								.findAll();

						JSONArray TypeSensorListToJson = new JSONArray(
								typeSensorList);

						toSend = TypeSensorListToJson.toString();
						break;
					case "FINDSENSITIVITY":
						TypeSensor typeSensor = typeSensorDao
								.findByType(parameters.getString("type"));

						toSend = typeSensor.toJSON().toString();
						break;
					case "FINDBYID":
						TypeSensor sensorFound = typeSensorDao.find(parameters
								.getInt("id"));
						toSend = sensorFound.toJSON().toString();
						break;
					default:
				...
				case "HISTORY_BREAKDOWN":
					...
					case "FINDALL":
						...
						JSONArray breakdownListToJson = new JSONArray(
								breakdownList);
					...
					case "FINDBYID":
						Breakdown breakdownFound = breakdownDao.find(parameters
								.getInt("id"));
						toSend = breakdownFound.toJSON().toString();
						break;

					case "CREATE":
						Breakdown breakdownToCreate = objectMapper.readValue(
								parameters.toString(), Breakdown.class);

						Boolean breakdownCreation = breakdownDao
								.create(breakdownToCreate);

						toSend = breakdownCreation.toString();
						break;

					case "DELETE":
						Boolean breakdownDeleting = breakdownDao
								.delete(parameters.getInt("id"));
					...
					case "UPDATE":
						Breakdown breakdownToUpdate = objectMapper.readValue(
								parameters.get("breakdownToUpdate").toString(),
								Breakdown.class);

						Boolean breakdownUpdating = breakdownDao.update(
								parameters.getInt("id"), breakdownToUpdate);
				...
				/**
				 * We write the result and send it to the ConnectionClient with
				 * .flush
				 */
				writer.write(toSend);
				writer.flush();
